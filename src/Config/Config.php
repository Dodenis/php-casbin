...
final class Config implements ConfigContract
{
    ...
    public const DEFAULT_COMMENT = '#';

    public const DEFAULT_COMMENT_SEM = ';';

    public const DEFAULT_MULTI_LINE_SEPARATOR = '\\';
    ...
    private function parse(string $fname): bool
    {
        ...
        return false === $buf ? false : $this->parseBuffer($buf);
    }
    ...
    private function parseBuffer(string $buf): bool
    {
        ...
        $buf = preg_replace('/[\r\n]+/', PHP_EOL, $buf);
        $buf = explode(PHP_EOL, null == $buf ? '' : $buf);
